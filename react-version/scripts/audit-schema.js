...
async function auditSchema() {
    ...
    if (!connectionString) {
        ...
        if (!supabaseUrl) {
            ...
            console.error('Get it from: Supabase Dashboard → Project Settings → Database → Connection Pooling\n');
            process.exit(1);
        }
...
async function auditWithDirectQueries(pool) {
    ...
    try {
        ...
        for (const table of tables) {
            ...
            console.log(`  ✓ ${columns.length} columns, ${indexes.length} indexes, ${foreignKeys.length} foreign keys, ${rowCount} rows\n`);
        }
        ...
        for (const tableName of Array.from(existingTables).sort()) {
            ...
            if (required) {
                ...
                const missingColumns = required.requiredColumns.filter(c => !existingColumns.includes(c));
        ...
        if (missingTables.length > 0) {
            ...
            report += 'These tables are required for the Jelly app but do not exist in the database:\n\n';
        ...
        if (missingTables.length > 0) {
            ...
            report += '2. Set up proper indexes for performance (especially on foreign keys and geo columns)\n';
