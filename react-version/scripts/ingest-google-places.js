...
async function getPlaceDetails(placeId) {
    ...
    try {
        const response = await axios.get(
            `${CONFIG.GOOGLE_API_BASE_URL}/places/${placeId}`,
            {
                headers: {
                    'Content-Type': 'application/json',
                    'X-Goog-Api-Key': CONFIG.GOOGLE_API_KEY,
        ...
        console.error(`  ❌ Error fetching details for ${placeId}:`, error.response?.data || error.message);
        throw error;
    }
}
...
async function downloadPhoto(photoName, maxWidth = CONFIG.PHOTO_MAX_WIDTH) {
    ...
    try {
        const response = await axios.get(
            `${CONFIG.GOOGLE_API_BASE_URL}/${photoName}/media`,
            {
                params: {
                    maxWidthPx: maxWidth,
...
async function processRestaurant(place, index, total) {
    ...
    try {
        ...
        // Process photos
        console.log(`  📸 Processing photos (${Math.min((placeDetails.photos || []).length, CONFIG.MAX_PHOTOS_PER_RESTAURANT)})...`);
...
async function main() {
    ...
    console.log(`  Location: Austin, TX (${CONFIG.LOCATION.latitude}, ${CONFIG.LOCATION.longitude})`);
